...
class FakePage:
    ...
    def locator(self, selector: str) -> FakeLocator:
        if selector not in self.locators:
            # Check if this is a combined selector containing the visible/throw selector
            is_visible = False
            throw_on_visible = False

            if self.visible_selector and f"{self.visible_selector}:visible" in selector:
                is_visible = True
            elif self.visible_selector == selector:
                is_visible = True

            if self.throw_on_selector and f"{self.throw_on_selector}:visible" in selector:
                throw_on_visible = True
            elif self.throw_on_selector == selector:
                throw_on_visible = True
...
def test_dismiss_popup_success():
    ...
    locator = list(page.locators.values())[-1]
