...
class FakePage:
    ...
    def locator(self, selector: str) -> FakeLocator:
        if selector not in self.locators:
            is_visible = self.visible_selector is not None and (self.visible_selector in selector or selector == self.visible_selector)
            throw_on_visible = self.throw_on_selector is not None and (self.throw_on_selector in selector or selector == self.throw_on_selector)
...
def test_dismiss_popup_success():
    ...
    # Verify it clicked the combined locator
    # We find the locator by checking which key contains the target_selector
    found_loc = None
    for k, loc in page.locators.items():
        if target_selector in k:
            found_loc = loc
            break
    assert found_loc is not None
    assert found_loc.clicked is True
