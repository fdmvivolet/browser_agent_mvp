...
class FakePage:
    ...
    def locator(self, selector: str) -> FakeLocator:
        if selector not in self.locators:
            # When multiple selectors are combined (e.g. '.close-button:visible, ...')
            # we check if our targeted visible_selector is in the combined selector.
            is_visible = (
                self.visible_selector is not None and self.visible_selector in selector
            )
            throw_on_visible = (
                self.throw_on_selector is not None
                and self.throw_on_selector in selector
            )
...
def test_dismiss_popup_success():
    ...
    page = FakePage(visible_selector=target_selector + ":visible")
    ...
    locator = list(page.locators.values())[0]
