...
class FakePage:
    ...
    def locator(self, selector: str) -> FakeLocator:
        if selector not in self.locators:
            # Check if any selector from the comma-separated list matches the visible_selector
            selectors = [s.strip() for s in selector.split(',')]
            is_visible = any(s == f"{self.visible_selector}:visible" or s == self.visible_selector for s in selectors) if self.visible_selector else False
            throw_on_visible = any(s == f"{self.throw_on_selector}:visible" or s == self.throw_on_selector for s in selectors) if self.throw_on_selector else False
...
def test_dismiss_popup_success():
    ...
    locator = next(iter(page.locators.values()))
