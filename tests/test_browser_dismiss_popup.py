...
class FakePage:
    ...
    def locator(self, selector: str) -> FakeLocator:
        if selector not in self.locators:
            is_visible = self.visible_selector is not None and self.visible_selector in selector
            throw_on_visible = self.throw_on_selector is not None and self.throw_on_selector in selector
...
def test_dismiss_popup_success():
    ...
    locator = next(loc for sel, loc in page.locators.items() if "combined-popup-selector" in sel or target_selector in sel)
