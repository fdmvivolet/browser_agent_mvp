...
class FakePage:
    ...
    def locator(self, selector: str) -> FakeLocator:
        if selector not in self.locators:
            ...
            if self.visible_selector and self.visible_selector in selector:
                is_visible = True
            if self.throw_on_selector and self.throw_on_selector in selector:
...
def test_dismiss_popup_success():
    ...
    # Verify it clicked a locator containing the target selector
    for sel, loc in page.locators.items():
        if target_selector in sel:
            assert loc.clicked is True
            break
    else:
        pytest.fail("Target selector not found in any clicked locator")
